...
const initApp = async  () => {
  ...
  const $voir = document.getElementById('voirUtilisateur');

  const $voirs = document.getElementById('voirUtilisateurs');
  ...
  $voir.addEventListener('submit', async (e) => {
    e.preventDefault();
    const data1 = e.target.elements[0].value;
    await Authentification.methods
      .voirUtilisateur(data1)
  ...
  $voirs.addEventListener('submit', async (e) => { 
    e.preventDefault();   
